...
type KLineStrategy struct {
	...
	MinAssetBalance float64 `json:"minAssetBalance"`
...
func (strategy *KLineStrategy) NewOrder(kline types.KLineOrWindow, tradingCtx *bbgo.TradingContext, detector *KLineDetector) (*types.SubmitOrder, error) {
	...
	case types.SideTypeBuy:
		...
		if balance, ok := tradingCtx.Balances[strategy.market.QuoteCurrency]; ok {
			...
			quantity = adjustQuantityByMinAmount(quantity, currentPrice, strategy.market.MinAmount*1.01)
	...
	case types.SideTypeSell:
		...
		if balance, ok := tradingCtx.Balances[strategy.market.BaseCurrency]; ok {
			if util.NotZero(strategy.MinAssetBalance) && balance.Available < strategy.MinAssetBalance {
				return nil, fmt.Errorf("base balance level is too low: %f > %f", balance.Available, strategy.MinAssetBalance)
			}

			quantity = adjustQuantityByMinAmount(quantity, currentPrice, strategy.market.MinNotional*1.01)
